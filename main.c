...
uint8_t pui8BufferIn[4]={0};
uint8_t pui8BufferOut[4]={4,5,6,7};
/*Threshold Values*/
uint8_t threshold[4] = { 50, 50, 50, 50 };
/*Fault Flags*/
uint8_t faultFlags = 0;
tCANBitClkParms CANBitClk={.ui32SyncPropPhase1Seg =11,.ui32Phase2Seg =3,.ui32SJW =1,.ui32QuantumPrescaler =32};

tCANMsgObject sMsgObjectRx={.ui32MsgID=0x01,.ui32MsgIDMask=0,.ui32MsgLen = 4 ,.ui32Flags = MSG_OBJ_USE_ID_FILTER,.pui8MsgData = pui8BufferIn};
tCANMsgObject sMsgObjectTx={.ui32MsgID = 0x01,.ui32Flags = 0,.ui32MsgLen = 4,.pui8MsgData = pui8BufferOut};
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if ((CANStatusGet(CAN0_BASE, CAN_STS_NEWDAT) & 1) == 1)
        {
            ...
            if (pui8BufferIn[0] == 0xF0 && pui8BufferIn[1] == 0xFE)
            {
